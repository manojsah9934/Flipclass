...
if (isset($_POST['login_user'])) {
	...
	if (count($errors) == 0) {
		$pass = md5($password);
		$query = "SELECT * FROM teacheregistration WHERE uname = '$uname' AND password = '$pass'";
		$results = mysqli_query($conn, $query);
		if(mysqli_num_rows($results) == 1) {
			$_SESSION['uname'] = $uname;
			$_SESSION['success'] = "You are now logged in";
			header('location: mainpage/Teacherportal.php');
		}
		else
		{
			...
			<div class="inner">
				...
				<form action="teacher_login.php" method = "post">
					...
					<button type = "submit" name = "login_user">Log In
						...
						<center> <b> Don't have an account? </b> <a href="teacher_registration.php" class = "ca">Create Account</a> </center>
				</form>
			</div>
		</div>
